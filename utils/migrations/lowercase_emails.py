...
def lowercase_emails_migration():
    ...
    for user in users:
        user_modal.update_one(
            {"_id": user["_id"]}, {"$set": {"email": user["email"].lower()}}
        )
...
def delete_duplicate_emails_migration():
        ...
        [
            ...
            },
            {
                "$project": {
                    "size": {"$size": "$emails"},
                    "emails": {
                        "$sortArray": {"input": "$emails", "sortBy": {"loggedInAt": -1}}
                    },
                }
            },
            {"$sort": {"size": -1}},
    ...
    for user in users:
        ...
        for i in user["emails"][1:]:
            ...
            deleted = user_modal.update_one(
                {"_id": i["userId"]}, {"$set": {"status": "deleted"}}
            )
