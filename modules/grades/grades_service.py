...
class GradesService:
    ...
    def course_weight_by_name(self, name: str) -> str:
        ...
        if (
            name.split().__contains__("honor")
            or name.split().__contains__("honors")
            or honors_weights.__contains__(name)
        ):
            return "honors"
    ...
    def query_live_gpa(self, genesisId, save=True):
        try:
            ...
            return {"unweightedGPA": None, "weightedGPA": None}
    ...
    def query_past_grades(self, genesisId):
        try:
            ...
            for key in courses.keys():
                ...
                calculated_gpa = self.calculate_gpa(
                    [None, gradeCourses, gradeWeights], manual_weights=grades
                )
                gpas.append(
                    {
                        **calculated_gpa,
                        "gradeLevel": key,
                        "year": courses[key][0]["year"],
                    }
                )
    ...
    def calculate_gpa(self, response, manual_weights=[]):
        ...
        course_loop = [
            ...
            if not ommitted_weights.__contains__(course["name"].lower())
            and not course["name"][0] == "*"
        ]
        ...
        for course in course_loop:
            ...
            if percentage and percentage != 0:
                ...
                if manually_set_weight is None:
                    if (
                        name.lower().split().__contains__("honor")
                        or name.lower().split().__contains__("honors")
                        or honors_weights.__contains__(name.lower())
                    ):
                        weighted_point = gpa_honors_points(percentage)
                    elif name.lower().split().__contains__(
                        "ap"
                    ) or ap_weights.__contains__(name.lower()):
        ...
        divisor = (
            course_points
            if course_points > 0
            else (len(course_loop) - excluded_courses)
        )
    ...
    def cleanup_classes(self, user, grades):
        ...
        for course_stored in courses_stored:
            ...
            for course in courses:
                if (
                    course["sectionId"] == course_stored["sectionId"]
                    and course["courseId"] == course_stored["courseId"]
                ):
                    course_exist = True
            if not course_exist:
    ...
    def send_assignment_update(self, token, assignment):
        ...
        message = (
            f"Scored {int(assignment['grade']['percentage'])}% on {assignment['name']}"
        )
    ...
    def send_grade_update(self, user, course, previous_percent, current_percent):
        ...
        if not notificationToken or previous_percent is None:
            return
    ...
    def send_gpa_update(self, user, gpa):
        ...
        try:
            fcm_service.send_message(
                token=notificationToken, message=message, title=f"GPA Update"
            )
        except Exception:
            pass
            ...
            try:
                if not change == None:
                    ...
                    if not previous_percent == current_percent:
                        low_queue.enqueue(
                            f=self.send_grade_update,
                            args=(user, course, previous_percent, current_percent),
                        )
    ...
    async def persist_assignments(self, user, persist_time):
        ...
        for i in response:
            serialized_new_assignments.append(
                json.dumps(
                    {
                        "name": i["name"],
                        "course": i["course"],
        ...
        for i in assignments:
            serialized_stored_assignments.append(
                json.dumps(
                    {
                        "name": i["name"],
                        "course": i["course"],
        ...
        if len(assignments):
            new_assignments = set(serialized_new_assignments) - set(
                serialized_stored_assignments
            )
            removed_assignments = set(serialized_stored_assignments) - set(
                serialized_new_assignments
            )
            if len(new_assignments):
                new_assignments = self.find_assignments(response, list(new_assignments))
                default_queue.enqueue_call(
                    func=self.store_assignments,
                    args=(user, new_assignments, send_notification),
                    description="store assignments",
                )
                default_queue.enqueue_call(
                    func=self.query_and_save_grades,
                    args=(genesisId, user),
                    description="query and save grades",
                    failure_ttl=60 * 60 * 24 * 2,
                )
            ...
            if len(removed_assignments):
                removed_assignments = self.find_assignments(
                    assignments, list(removed_assignments)
                )
                default_queue.enqueue_call(
                    func=self.clean_assignments,
                    args=(
                        user["_id"],
                        removed_assignments,
                    ),
                    description="clean assignments",
                )
        else:
            send_notification = False
            default_queue.enqueue_call(
                func=self.store_assignments,
                args=(user, response, send_notification),
                description="store assignments",
            )
        ...
        del assignments
        del serialized_new_assignments
        del serialized_stored_assignments
    ...
    def query_grades(self, skip):
        ...
        try:
            ...
            if returned_total == 0 or returned_total is None:
                default_queue.enqueue_call(
                    func=self.save_persist_time,
                    args=(persist_time,),
                    description="persist time",
                )
                next_skip = 0
            ...
            if next_skip != 0:
                del response
                default_queue.enqueue_call(
                    func=self.query_grades,
                    args=(next_skip,),
                    description="query grades",
                    failure_ttl=60 * 60 * 24 * 2,
                )
