...
queue = Queue("low", connection=conn) # change queue to `default` if you want to clean up keys in `default`
...
for i in range(max_workers): 
    pool.submit(
        clean_up_jobs, 
        failed_job_ids[equal_jobs*i:equal_jobs*(i+1) if i != max_workers - 1 else -1]
    )
